...
import { deleteKeyAlias } from '../commands/deleteKey.ts';
...
export function registerDoctorCommand(program: Command) {
  program.command('doctor')
    .description('Checks SSH config for missing key files and offers to clean up.')
    .action(async () => {
      ...
      if (problematicEntries.length === 0) {
        console.log('No missing SSH key files found in your config. Your SSH config is healthy!');
        return;
      }

      console.log('Found missing SSH key files for the following hosts:');
      for (const entry of problematicEntries) {
        console.log(`- Host: ${entry.host}, Missing IdentityFile: ${entry.identityFile}`);
      }
      ...
      for (const entry of problematicEntries) {
        ...
        if (confirmDelete === 'Yes') {
          // We need to call the internal deleteKeyAlias function from deleteKey.ts
          // Since it's not directly exported, we might need to refactor deleteKey.ts
          // to export it, or duplicate the logic here. For now, I'll assume refactoring.
          // For now, I'll call the default export and pass the host as keyName
